 ...
 * Calls: create_listing(seller: Address, title: String, description: String, price: i128, token: Address) -> u64
 */
...
/**
 * Purchase a listing. Buyer must send payment directly to the seller.
 * Calls: purchase(buyer: Address, listing_id: u64)
 */
export async function purchase(caller: string, listingId: number) {
  return callContract(
    "purchase",
    [toScValAddress(caller), toScValU64(BigInt(listingId))],
    caller,
    true
  );
}
 ...
 * Calls: mark_shipped(seller: Address, listing_id: u64)
 */
export async function markShipped(caller: string, listingId: number) {
  return callContract(
    "mark_shipped",
    [toScValAddress(caller), toScValU64(BigInt(listingId))],
    caller,
 ...
 * Calls: confirm_received(buyer: Address, listing_id: u64)
 */
export async function confirmReceived(caller: string, listingId: number) {
  return callContract(
    "confirm_received",
    [toScValAddress(caller), toScValU64(BigInt(listingId))],
    caller,
...
export async function getListing(listingId: number) {
  return readContract("get_listing", [toScValU64(BigInt(listingId))]);
}

/**
 * Get all listings.
 * Calls: get_all_listings() -> Vec<Listing>
 */
export async function getAllListings() {
  return readContract("get_all_listings", []);
}

/**
 * Get only available (unpurchased) listings.
 * Calls: get_available_listings() -> Vec<Listing>
 */
export async function getAvailableListings() {
  return readContract("get_available_listings", []);
}
